...
void mmAttachmentDialog::fillControls()
{
    ...
    for ( const auto &entry : attachments )
    {
        ...
        {
            firstInTheListAttachentID = entry.ATTACHMENTID;
        }
        ...
        if ( debug_ )
        {
            data.push_back ( wxVariant ( wxString::Format ( "%i", entry.ATTACHMENTID ) ) );
        }
...
void mmAttachmentDialog::OnMenuSelected ( wxCommandEvent &event )
{
    switch ( event.GetId() )
    {
        ...
        case MENU_EDIT_ATTACHMENT:
            EditAttachment();
            break;

        case MENU_DELETE_ATTACHMENT:
            DeleteAttachment();
            break;
...
void mmAttachmentDialog::OnItemRightClick ( wxDataViewEvent &event )
{
    ...
    if ( attachment )
    {
        mainMenu->SetTitle ( attachment->DESCRIPTION );
    }
