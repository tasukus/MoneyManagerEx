...
void mmAssetsListCtrl::OnColClick ( wxListEvent &event )
{
    ...
    if ( 0 > ColumnNr || ColumnNr >= m_panel->getColMax( ) || ColumnNr == 0 )
    {
        return;
    }
...
void mmAssetsPanel::sortTable()
{
    ...
    if ( !this->m_listCtrlAssets->m_asc )
    {
        std::reverse ( this->m_assets.begin( ), this->m_assets.end( ) );
    }
}
...
{
    const int evt = std::max ( event.GetId( ) - 1, 0 );
