...
void mmBDDialog::OnOk ( wxCommandEvent &WXUNUSED ( event ) )
{
    ...
    if ( !Model_Billsdeposits::instance().AllowTransaction ( bill_data, bal ) )
    {
        return;
    }
    ...
    if ( m_transfer )
    {
        ...
        if ( m_advanced && !toTextAmount_->checkValue ( m_bill_data.TOTRANSAMOUNT ) )
        {
            return;
        }
    }
