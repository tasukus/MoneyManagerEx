...
#include <wx/sound.h>
//----------------------------------------------------------------------------
std::unordered_map<mmCheckingPanel::menu, wxString> mmCheckingPanel::menu_labels =
{
    {mmCheckingPanel::MENU_VIEW_ALLTRANSACTIONS,VIEW_TRANS_ALL_STR},
    {mmCheckingPanel::MENU_VIEW_TODAY,VIEW_TRANS_TODAY_STR},
    {mmCheckingPanel::MENU_VIEW_CURRENTMONTH,VIEW_TRANS_CURRENT_MONTH_STR},
    {mmCheckingPanel::MENU_VIEW_LAST30,VIEW_TRANS_LAST_30_DAYS_STR},
    {mmCheckingPanel::MENU_VIEW_LAST90,VIEW_TRANS_LAST_90_DAYS_STR},
    {mmCheckingPanel::MENU_VIEW_LASTMONTH,VIEW_TRANS_LAST_MONTH_STR},
    {mmCheckingPanel::MENU_VIEW_LAST3MONTHS,VIEW_TRANS_LAST_3MONTHS_STR},
    {mmCheckingPanel::MENU_VIEW_LAST12MONTHS,VIEW_TRANS_LAST_12MONTHS_STR},
    {mmCheckingPanel::MENU_VIEW_CURRENTYEAR,VIEW_TRANS_CURRENT_YEAR_STR},
    {mmCheckingPanel::MENU_VIEW_CURRENTFINANCIALYEAR,VIEW_TRANS_CRRNT_FIN_YEAR_STR},
    {mmCheckingPanel::MENU_VIEW_LASTYEAR,VIEW_TRANS_LAST_YEAR_STR},
    {mmCheckingPanel::MENU_VIEW_LASTFINANCIALYEAR,VIEW_TRANS_LAST_FIN_YEAR_STR},
    {mmCheckingPanel::MENU_VIEW_STATEMENTDATE,VIEW_TRANS_SINCE_STATEMENT_STR},
    {mmCheckingPanel::MENU_VIEW_FILTER_DIALOG,VIEW_TRANS_FILTER_DIALOG_STR},
};
//----------------------------------------------------------------------------
...
wxBEGIN_EVENT_TABLE ( TransactionListCtrl, mmListCtrl )
    ...
    EVT_LIST_ITEM_DESELECTED ( wxID_ANY, TransactionListCtrl::OnListItemUnSelected )
...
mmCheckingPanel::mmCheckingPanel ( wxWindow *parent, mmGUIFrame *frame, int accountID, int id )
    ...
    , m_frame ( frame )
{
    constexpr long style = wxTAB_TRAVERSAL | wxNO_BORDER;
...
bool mmCheckingPanel::Create (wxWindow *parent,wxWindowID winid, const wxPoint &pos,const wxSize &size,long style, const wxString &name
)
{
    ...
    m_trans_filter_dlg = std::make_unique<mmFilterTransactionsDialog> ( this, m_AccountID );
...
void mmCheckingPanel::sortTable()
{
    switch ( m_listCtrlAccount->g_sortcol )
    {
        ...
        case TransactionListCtrl::COL_IMGSTATUS:
            break;
...
void mmCheckingPanel::filterTable()
{
    ...
    const int udfc01_ref_id = matrix.at ( "UDFC01" );
    const int udfc02_ref_id = matrix.at ( "UDFC02" );
    const int udfc03_ref_id = matrix.at ( "UDFC03" );
    const int udfc04_ref_id = matrix.at ( "UDFC04" );
    const int udfc05_ref_id = matrix.at ( "UDFC05" );
    ...
    {
        const double transaction_amount = Model_Checking::amount ( tran, m_AccountID );
        ...
        else
        {
            ...
            {
                if ( tran.TRANSDATE < m_begin_date )
                {
                    continue;
                }

                if ( tran.TRANSDATE > m_end_date )
                {
                    continue;
                }
            }
...
void mmCheckingPanel::markSelectedTransaction ( int trans_id )
{
    ...
    if ( !m_trans.empty() && m_listCtrlAccount->m_selectedIndex < 0 )
    {
        ...
        if ( m_listCtrlAccount->g_sortcol == TransactionListCtrl::COL_DATE )
        {
            ...
            while ( future && i >= 0 )
            {
                if ( m_trans.at ( i ).TRANSDATE > m_listCtrlAccount->m_today&& i > 0 )
                {
        ...
        m_listCtrlAccount->m_selectedIndex = i;
        m_listCtrlAccount->m_selectedID = m_trans.at ( i ).TRANSID;
    }
    else
    {
...
void mmCheckingPanel::OnMouseLeftDown ( wxCommandEvent &event )
{
    ...
    {
        menu.Append ( id++, wxGetTranslation ( i.second ) );
    }
...
void mmCheckingPanel::CreateControls()
{
    ...
    itemBoxSizerHHeader2->Add ( m_bitmapTransFilter, g_flagsBorder1V );
...
{
    const Model_Account::Data *account = Model_Account::instance().get ( m_AccountID );
...
void mmCheckingPanel::setAccountSummary()
{
    ...
    const bool show_displayed_balance_ = ( m_transFilterActive || m_currentView != MENU_VIEW_ALLTRANSACTIONS );
...
void mmCheckingPanel::updateExtraTransactionData ( int selIndex )
{
    if ( selIndex > -1 )
    {
        ...
        const Model_Checking::Full_Data &full_tran = this->m_trans.at ( selIndex );
...
{
    const wxString &text = TIPS[ rand() % std::size ( TIPS ) ];
    const wxString tipText = wxGetTranslation ( text );
    m_info_panel->SetLabelText ( tipText );
}
//----------------------------------------------------------------------------
...
void mmCheckingPanel::OnViewPopupSelected ( wxCommandEvent &event )
{
    //--------------------------
    {
        const int index = event.GetId() - wxID_HIGHEST;
        m_currentView = static_cast<enum menu> ( index );
    }
    //--------------------------
    if ( m_currentView == MENU_VIEW_FILTER_DIALOG )
    {
    ...
    const wxString &item = menu_labels.at ( m_currentView );
    ...
    DisplayAccountDetails ( m_AccountID );
}
...
void mmCheckingPanel::DeleteViewedTransactions()
{
    ...
    for ( const auto &tran: this->m_trans )
    {
        ...
        if ( m_listCtrlAccount->m_selectedForCopy == tran.TRANSID )
        {
...
const wxString mmCheckingPanel::getItem ( const long item, const TransactionListCtrl::EColumn column )
{
    if ( item < 0 || static_cast<size_t> ( item ) >= m_trans.size() )
    {
        return "";
    }
    ...
    while ( selectedItem >= 0 && selectedItem <= last )
    {
            ...
            } )
        {
            if ( t.Lower().Matches ( search_string + "*" ) )
            {
                ...
                const long cursel = m_listCtrlAccount->GetNextItem ( -1
...
void mmCheckingPanel::DisplaySplitCategories (const int transID )
{
    const Model_Checking::Data *tran = Model_Checking::instance().get ( transID );
    const int transType = Model_Checking::type ( tran->TRANSCODE );
    ...
    {
        const Split s ( entry.CATEGID, entry.SUBCATEGID, entry.SPLITTRANSAMOUNT );
        splt.push_back ( s );
    }

    SplitTransactionDialog splitTransDialog(this , splt , transType , m_AccountID);

    splitTransDialog.SetDisplaySplitCategories();
    splitTransDialog.ShowModal();
}

void mmCheckingPanel::RefreshList (const int transID )
{
    m_listCtrlAccount->refreshVisualList ( transID );
}

void mmCheckingPanel::SetSelectedTransaction (const int transID )
{
    RefreshList ( transID );
    m_listCtrlAccount->SetFocus();
}
...
void mmCheckingPanel::DisplayAccountDetails ( const int accountID )
{
    m_AccountID = accountID;
    m_account = Model_Account::instance().get ( accountID );
...
{
    const bool play = Model_Setting::instance().GetBoolSetting ( INIDB_USE_TRANSACTION_SOUND, true );
...
TransactionListCtrl::TransactionListCtrl(mmCheckingPanel *cp , wxWindow *parent , const wxWindowID id)
    ...
    m_attr17 ( *wxYELLOW, mmColors::userDefColor7, wxNullFont )
{
    ...
    wxAcceleratorTable tab ( static_cast<int> ( std::size ( entries ) ), entries );
    ...
    for ( const auto &udfc_entry : Model_CustomField::UDFC_FIELDS() )
    {
        ...
        const int width = name == udfc_entry ? 0 : 100;
...
void TransactionListCtrl::createColumns ( mmListCtrl &lst )
{
    ...
    {
        const int count = lst.GetColumnCount();
...
void TransactionListCtrl::OnListItemSelected ( wxListEvent &event )
{
    ...
    m_selectedID = m_cp->m_trans.at ( m_selectedIndex ).TRANSID;

    updateInfoPanel();
    return;
}
void TransactionListCtrl::OnListItemUnSelected ( wxListEvent &WXUNUSED ( event ) )
{
    m_selectedIndex = -1;
...
void TransactionListCtrl::OnMouseRightClick ( wxMouseEvent &event )
{
    ...
    const bool hide_menu_item = ( m_selectedIndex < 0 );
    const bool multiselect = ( GetSelectedItemCount() > 1 );
...
void TransactionListCtrl::OnMarkTransaction ( wxCommandEvent &event )
{
    ...
    {
        wxFAIL_MSG ( "unknown transaction status" );
    }

    Model_Checking::Full_Data &trx = m_cp->m_trans.at ( m_selectedIndex );
    ...
    const bool bRefreshRequired = ( status == "V" ) || ( org_status == "V" );

    if ( ( m_cp->m_transFilterActive && m_cp->m_trans_filter_dlg->getStatusCheckBox() )
        || bRefreshRequired )
    {
        refreshVisualList ( m_cp->m_trans.at ( m_selectedIndex ).TRANSID );
    }
    else
    {
...
void TransactionListCtrl::OnMarkAllTransactions ( wxCommandEvent &event )
{
    ...
    else if ( evt == MENU_TREEPOPUP_DELETE_VIEWED )
    {
        status = "X";
    }
    else if ( evt == MENU_TREEPOPUP_DELETE_FLAGGED )
    {
        status = "M";
    }
    else if ( evt == MENU_TREEPOPUP_DELETE_UNRECONCILED )
    {
        status = "0";
    }
    else
...
void TransactionListCtrl::OnColClick ( wxListEvent &event )
{
    ...
    if ( ColumnNr < 0 || static_cast<size_t> ( ColumnNr ) >= m_columns.size() || ColumnNr == COL_IMGSTATUS )
    {
        return;
    }
...
{
    return m_cp->getItem ( item, static_cast<TransactionListCtrl::EColumn> ( column ) );
}
//----------------------------------------------------------------------------
...
wxListItemAttr *TransactionListCtrl::OnGetItemAttr ( long item ) const
{
    ...
    const Model_Checking::Full_Data &tran = m_cp->m_trans.at ( item );
    const bool in_the_future = ( tran.TRANSDATE > m_today );
...
void TransactionListCtrl::OnCopy ( wxCommandEvent &WXUNUSED ( event ) )
{
    ...
    {
        m_selectedForCopy = m_cp->m_trans.at ( m_selectedIndex ).TRANSID;
    }
...
void TransactionListCtrl::OnDuplicateTransaction ( wxCommandEvent &WXUNUSED ( event ) )
{
    ...
    const int transaction_id = m_cp->m_trans.at ( m_selectedIndex ).TRANSID;
...
int TransactionListCtrl::OnPaste ( Model_Checking::Data *tran )
{
    ...
    const  int transactionID = Model_Checking::instance( ).save ( copy );
...
void TransactionListCtrl::OnListKeyDown ( wxListEvent &event )
{
    ...
    const wxString &status = m_cp->m_trans.at ( m_selectedIndex ).STATUS;
...
void TransactionListCtrl::OnDeleteTransaction ( wxCommandEvent &WXUNUSED ( event ) )
{
    ...
    Model_Checking::Data checking_entry = m_cp->m_trans.at ( m_selectedIndex );
    ...
    if ( msgDlg.ShowModal() == wxID_YES )
    {
        ...
        for ( const auto &i : m_cp->m_trans )
        {
            ...
            if ( GetItemState ( x, wxLIST_STATE_SELECTED ) == wxLIST_STATE_SELECTED )
            {
                ...
                if ( !m_cp->m_trans.empty() && m_selectedIndex > 0 )
                {
                    m_selectedIndex--;
                }

                if ( m_selectedForCopy == transID )
                {
                    m_selectedForCopy = -1;
                }
            }
...
void TransactionListCtrl::OnEditTransaction ( wxCommandEvent &WXUNUSED ( event ) )
{
    ...
    Model_Checking::Data checking_entry = m_cp->m_trans.at ( m_selectedIndex );
    const int transaction_id = checking_entry.TRANSID;
...
{
    const int type = event.GetId() == MENU_TREEPOPUP_NEW_DEPOSIT ? Model_Checking::DEPOSIT : Model_Checking::WITHDRAWAL;
...
void TransactionListCtrl::OnSetUserColour ( wxCommandEvent &event )
{
    ...
    if ( transaction )
    {
        ...
        m_cp->m_trans.at ( m_selectedIndex ).FOLLOWUPID = user_colour_id;
        RefreshItems ( m_selectedIndex, m_selectedIndex );
    }
}
//----------------------------------------------------------------------------

void TransactionListCtrl::refreshVisualList ( const int trans_id, bool filter )
{
    m_today = wxDateTime::Today().FormatISODate();
    ...
    const long i = static_cast<long> ( m_cp->m_trans.size( ) );
    ...
    if ( m_selectedIndex > i - 1 )
    {
        m_selectedIndex = -1;
    }

    if ( m_topItemIndex < m_selectedIndex )
    {
...
void TransactionListCtrl::OnMoveTransaction ( wxCommandEvent &WXUNUSED ( event ) )
{
    ...
    for ( const long index : GetSelected() )
    {
        Model_Checking::Data checking_entry = m_cp->m_trans.at ( index );
    ...
    if ( scd.ShowModal() == wxID_OK )
    {
        ...
        const Model_Account::Data *dest_account = Model_Account::instance().get ( dest_account_name );
        ...
        for ( const long index : GetSelected() )
        {
            const Model_Checking::Full_Data &tran = m_cp->m_trans.at ( index );
...
void TransactionListCtrl::OnOrganizeAttachments ( wxCommandEvent &WXUNUSED ( event ) )
{
    ...
    const wxString &RefType = Model_Attachment::reftype_desc ( Model_Attachment::TRANSACTION );
    const int RefId = m_cp->m_trans.at ( m_selectedIndex ).TRANSID;
...
void TransactionListCtrl::OnCreateReoccurance ( wxCommandEvent &WXUNUSED ( event ) )
{
    ...
    dlg.SetDialogParameters ( m_cp->m_trans.at ( m_selectedIndex ) );
...
void TransactionListCtrl::updateInfoPanel()
{
    m_cp->enableEditDeleteButtons ( true );

    const long count = this->GetSelectedItemCount();
    ...
    for ( const auto &i : m_cp->m_trans )
    {
        if ( GetItemState ( x, wxLIST_STATE_SELECTED ) == wxLIST_STATE_SELECTED )
        {
            ...
            if ( minDate > i.TRANSDATE || maxDate.empty() )
            {
                minDate = i.TRANSDATE;
            }

            if ( maxDate < i.TRANSDATE || maxDate.empty() )
            {
                maxDate = i.TRANSDATE;
            }
        }
    ...
    const int days = max_date.Subtract ( min_date ).GetDays();

    wxString msg;
    const Model_Account::Data *account = Model_Account::instance().get ( m_cp->m_AccountID );
