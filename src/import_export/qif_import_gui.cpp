...
bool mmQIFImportDialog::completeTransaction ( std::unordered_map <int, wxString> &trx, const wxString &accName )
{
    ...
    if ( trx.find ( Category ) != trx.end() )
    {
        ...
        if ( !categ_suffix.empty() )
        {
            trx[Memo] += ( trx[Memo].empty() ? "" : "/n" ) + categ_suffix;
        }
...
bool mmQIFImportDialog::mergeTransferPair ( Model_Checking::Cache &to, Model_Checking::Cache &from )
{
    ...
    for ( auto &refTrxTo : to )
    {
        ...
        {
            ++i;
            if ( refTrxTo->ACCOUNTID != refTrxFrom->TOACCOUNTID )
            {
                continue;
            }
            if ( refTrxTo->TOACCOUNTID != refTrxFrom->ACCOUNTID )
            {
                continue;
            }
            if ( refTrxTo->TRANSACTIONNUMBER != refTrxFrom->TRANSACTIONNUMBER )
            {
                continue;
            }
            if ( refTrxTo->NOTES != refTrxFrom->NOTES )
            {
                continue;
            }
            if ( refTrxTo->TRANSDATE != refTrxFrom->TRANSDATE )
            {
                continue;
            }
...
void mmQIFImportDialog::getOrCreateCategories()
{
    ...
    for ( const auto &item : m_QIFcategoryNames )
    {
        ...
        if ( temp.Index ( categStr ) != wxNOT_FOUND )
        {
            continue;
        }
